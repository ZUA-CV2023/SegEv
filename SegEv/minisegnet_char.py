    ...
    def make_priors(self, conv_h, conv_w, device):
        ...
        with timer.env('makepriors'):
            if self.last_img_size != (cfg._tmp_img_w, cfg._tmp_img_h):
                ...
                self.priors = torch.tensor(torch.Tensor(prior_data), device='cuda').view(-1, 4).detach()
    ...
    def forward(self, x):
        ...
        if cfg.mask_type == mask_type.lincomb and cfg.eval_mask_branch:
            with timer.env('proto'):
                ...
                charc = proto_out[0].cpu().detach().numpy()
                for i in range(32):
                    plt.subplot(4, 8, i + 1)
                    plt.imshow(charc[:,:,i])
                    plt.title("")
                    plt.xticks(), plt.yticks()
                output_dir = "./feature_testfiles"
                if not os.path.exists(output_dir):
                    os.makedirs(output_dir)
                path_feature_img = os.path.join(output_dir, "feature_map"  + ".png")
        ...
        if self.training:
            ...
            result = self.detect(pred_outs, self)
            return result[0]["detection"]['box']
